...
function Controller(){
    ...
    this.placeOrder = async function (req, res) {
        try {
            const { userId, items, totalAmount, lat, long, paymentMethod, sellerId } = req.body;
            // console.log(req.body);return;
            // Validate required fields
            if (!userId || !items || !totalAmount || !lat || !long || !paymentMethod) {
            ...
            for (const item of items) {
                if (!item.categoryId || !item.subCategoryId || !item.productName ||
                    !item.quantity || !item.dishPrice || !item.gst || !item.packagePrice || !item.finalDishPrice) {
            ...
            // let calculatedTotal = items.reduce((sum, item) => sum + item.finalDishPrice * item.quantity, 0);
            ...
            return Responder.sendSuccess(res, "Order placed successfully", 201, order);
    ...
    this.acceptOrder = async function (req, res) {
        try {
            ...
            if (!partners.data || partners.data.allPartners.length === 0) {
                return Responder.sendFailure(res, "No delivery partners available", 404);
            }
            ...
            return Responder.sendSuccess(res, "Order accepted, delivery partner notified", 200);
    ...
    this.trackOrder = async function (req, res) {
        try {
            ...
            if (!order.deliveryPartnerId) {
                return Responder.sendFailure(res, "Delivery partner not assigned yet", 400);
            }
    
            let deliveryPartner = await PartnerModel.findOne({ partnerId: order.deliveryPartnerId });
    
            if (!deliveryPartner) {
                return Responder.sendFailure(res, "Delivery partner details not found", 404);
            }
            ...
            return Responder.sendSuccess(res, "Route fetched successfully", 200, response.data.routes);
