...
function Controller() {
    ...
    if (!areaId || areaId == undefined || areaId == null) {
      ...
      if (updatedResult) {
        return Responder.sendSuccess(
          res,
          "Area Mapping Updated Successfully",
          200
        );
      } else {
  ...
  this.multipleMapingUpdate = async function (req, res) {
    try {
      ...
      let areaMap = new Map(
        allExistingAreas.map((area) => [area.areaId, area])
      );
  ...
  this.verifyOtp = async function (req, res, type) {
    ...
    try {
      ...
      if (!phone || !otp)
        return Responder.sendFailure(res, "Phone and OTP are required", 400);
      ...
      if (storedOtpData.otp !== otp)
        return Responder.sendFailure(res, "Invalid OTP", 401);
      ...
      return Responder.sendSuccess(
        res,
        "OTP verified successfully. Please provide name and location",
        200,
        { token, user }
      );
    } catch (error) {
    ...
    try {
      ...
      if (location && locationType) {
        if (!["home", "office", "others"].includes(locationType)) {
          return Responder.sendFailure(
            res,
            "Invalid location type. Use 'home', 'office', or 'others'",
            400
          );
        }
        ...
        user.location[locationType] = {
          ...
          doorNo: location.doorNo || user.location[locationType]?.doorNo,
          streetName:
            location.streetName || user.location[locationType]?.streetName,
          landmark: location.landmark || user.location[locationType]?.landmark,
        };
      }
  ...
  this.findZonesContainingDeliveryPartner = async function (res, req) {
    try {
      ...
      //   console.log("matchedArea", matchedArea, "matchedArea");
      //   return;
      if (!matchedArea) {
        return Responder.sendFailure(
          res,
  ...
  this.categoryAddFromSeller = async function (req, res) {
    try {
      ...
      if (result) {
        return Responder.sendSuccess(
          res,
          "Category Added Successfully, Waiting for the Approval",
          201,
          result
        );
      } else {
  ...
  this.subCategoryAddFromSeller = async function (req, res) {
    try {
      ...
      if (result) {
        return Responder.sendSuccess(
          res,
          "Subcategory Added Successfully, Waiting for Approval",
          201,
          result
        );
      } else {
  ...
  this.getSubcategoriesBySeller = async function (req, res) {
    try {
      ...
      if (!sellerId || !categoryId) {
        return Responder.sendFailure(
          res,
          "Seller ID and Category ID are required",
          400
        );
      }
      ...
      if (!subcategories || subcategories.length === 0) {
        return Responder.sendFailure(
          res,
          "No subcategories found for this seller and category",
          404
        );
      }
